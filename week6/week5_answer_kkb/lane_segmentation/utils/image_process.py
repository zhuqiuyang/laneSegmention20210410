...
class LaneDataset(Dataset):
    ...
    def __init__(self, csv_file, lmdb_dir, transform=None):
    ...
    def __getitem__(self, idx):
        # 通过idx从数据目录csv文件中获取对应行（图片名称）
        image_name = self.name_list[idx].split(',')[0]
        label_name = self.name_list[idx].split(',')[1].replace('\n', '')
        # 图片名称编码后在字典中查找值
        image = self.txn.get(image_name.encode())
        label = self.txn.get(label_name.encode())
        # 数据格式转换
        image = np.frombuffer(image, dtype=np.uint8)
        label = np.frombuffer(label, dtype=np.uint8)
        image = cv2.imdecode(image, cv2.IMREAD_COLOR)
        label = cv2.imdecode(label, cv2.IMREAD_GRAYSCALE)
