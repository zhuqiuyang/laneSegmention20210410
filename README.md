...
作业：
    下载数据集，配置GPU训练环境，跑通老师提供的代码作为Baseline
    在Baseline基础上，尝试1-3种优化方案，提升模型准确率
        提示：可以从优化器，学习率策略，损失函数，样本均衡，数据扩增，多分辨率，模型融合等方面优化
提交内容：
    Baseline跑通截图
    README.txt -- 说明优化方案的设计思路及实施细节，同时指明相关代码的文件名及行数（如：在train.py的23-26行进行了第一种优化)
    修改后的项目代码（只提交代码，不要提交模型文件)
    验证集最优结果截图
文件夹内容说明：
