...
class MySoftmaxCrossEntropyWeightedLoss(nn.Module):

    def __init__(self, nbclasses):
        super(MySoftmaxCrossEntropyWeightedLoss, self).__init__()
        self.nbclasses = nbclasses
        self.weight = torch.tensor([2.0532e-04, 1.5655e-02, 6.7465e-02, 1.7170e-01,
                                    4.9341e-01, 1.8606e-02, 5.2471e-02, 1.8049e-01], dtype=torch.float32)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target):
        target = make_one_hot(target.unsqueeze(1), inputs.size(1))
        inter = (inputs * target).sum(-1).sum(-1)
        summation = (inputs + target).sum(-1).sum(-1)
